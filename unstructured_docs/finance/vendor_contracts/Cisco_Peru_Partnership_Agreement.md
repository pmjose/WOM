# Cisco Chile Partnership Agreement
**WOM Communications PLC - Technology Alliance**

## Agreement Summary

| Field | Details |
|-------|---------|
| Agreement Type | Technology Partner Agreement |
| Parties | WOM Communications PLC / Cisco Systems Chile Ltd |
| Effective Date | 1 April 2022 |
| Term | 5 years |
| Total Commitment | $8M (technology + services) |

---

## Partnership Scope

### Products Covered

| Product | Description | WOM Role |
|---------|-------------|------------|
| Webex for WOM | Fibra platform | Reseller/Partner |
| Cisco BroadWorks | Platform licensing | Licensee |
| Webex Contact Centre | Internet Empresas solution | Reseller |
| Cisco Networking | Infrastructure | Reseller |

### Partnership Tier

**Cisco Gold Partner**
- Specialisations: Collaboration, Enterprise Networking
- Benefits: Deal registration, MDF, training credits

---

## Commercial Terms

### Pricing Structure

| Product | Discount Tier | Minimum Volume |
|---------|---------------|----------------|
| Webex Calling | 45% | 10,000 subscribers |
| Webex Meetings | 40% | 5,000 subscribers |
| Contact Centre | 35% | 1,000 subscribers |
| Network Hardware | 30% | $100K/year |

### Volume Commitments

| Year | Webex Revenue | Hardware | Services |
|------|---------------|----------|----------|
| 2024 | $3.5M | $500K | $200K |
| 2025 | $4.5M | $600K | $300K |
| 2026 | $5.5M | $700K | $400K |

---

## Technical Requirements

### Platform Integration

| Integration | Status | Description |
|-------------|--------|-------------|
| BroadWorks Platform | Complete | Core Fibra |
| Webex Control Hub | Complete | Management |
| API Integration | Complete | Provisioning |
| SSO/SAML | Complete | Authentication |

### Interoperability

| Requirement | WOM Commitment |
|-------------|------------------|
| SIP interconnect | Maintain certified |
| Codec support | Full suite |
| Emergency services | Chile compliant |
| Number portability | Full support |

---

## Support and Maintenance

### Support Model

| Level | Response Time | Resolution |
|-------|---------------|------------|
| P1 - Critical | 15 minutes | 4 hours |
| P2 - Major | 1 hour | 8 hours |
| P3 - Minor | 4 hours | 24 hours |
| P4 - Low | 8 hours | 72 hours |

### Escalation Path

| Level | Cisco Contact | WOM Contact |
|-------|---------------|---------------|
| 1 | TAC | NOC |
| 2 | TAC Manager | Ops Director |
| 3 | Account SE | CTO |
| 4 | VP Engineering | CEO |

---

## Marketing and Enablement

### Marketing Development Funds

| Activity | Allocation | Usage |
|----------|------------|-------|
| Campaigns | $150K/year | Demand gen |
| Events | $75K/year | Partner events |
| Content | $50K/year | Collateral |
| Training | $75K/year | Certifications |

### Training Requirements

| Certification | Requirement | Validity |
|---------------|-------------|----------|
| Webex Sales | 10 staff | 2 years |
| Webex Technical | 5 engineers | 2 years |
| Contact Centre | 3 specialists | 2 years |
| Network | 5 engineers | 2 years |

---

## Governance

### Joint Business Planning

| Review | Frequency | Focus |
|--------|-----------|-------|
| Executive | Quarterly | Strategy, investment |
| Business | Monthly | Pipeline, forecast |
| Technical | Bi-weekly | Issues, roadmap |
| Marketing | Monthly | Campaigns, MDF |

### Key Contacts

| Function | Cisco | WOM |
|----------|-------|-------|
| Account Lead | Partner Manager | Alliance Director |
| Technical | Solutions Architect | CTO |
| Marketing | Channel Marketing | Marketing Director |
| Operations | Partner Ops | Ops Director |

---

## Risk and Compliance

### Compliance Requirements

| Standard | Status | Owner |
|----------|--------|-------|
| ISO 27001 | Certified | Both |
| SOC 2 | Certified | Both |
| GDPR | Compliant | Both |
| Chile Telecom | Licensed | WOM |

### Audit Rights

- Annual compliance audit
- Security assessment bi-annual
- Financial audit rights
- Service quality reviews

---

## Renewal and Termination

### Contract Term

- Initial term: 5 years (to April 2027)
- Auto-renewal: 12-month periods
- Notice period: 12 months for termination

### Exit Provisions

| Scenario | Process |
|----------|---------|
| End of term | 12-month wind-down |
| Breach | 90-day cure period |
| Change of control | 6-month review |

---

**Document Classification**: Commercial in Confidence  
**Contract Owner**: Commercial Director  
**Last Review**: January 2025  
**Next Review**: April 2025
